...
## Local Setup (New Developer)

If a developer says they need to set up the project locally or asks you to create `.env.local`, follow these steps:

1. **Check for `.env.local`** — if it doesn't exist, offer to create it.
2. **Ask for the secret values** — tell the developer: *"Please paste the values from the handoff PDF (Tees-House-Site-Handoff.pdf) and I'll create the file for you."*
3. **Create `.env.local`** in the project root with these exact keys:
...
5. **Start dev server** — run `npm run dev` — site is live at http://localhost:3000
6. **Deploy** — any `git push origin main` auto-deploys to Vercel (no manual step needed)
